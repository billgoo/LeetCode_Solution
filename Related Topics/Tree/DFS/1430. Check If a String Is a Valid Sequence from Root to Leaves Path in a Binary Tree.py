...
Hint #1  
Depth-first search (DFS) with the parameters: current node in the binary tree and current 
position in the array of integers.
...
class Solution:
    def isValidSequence(self, root: TreeNode, arr: List[int]) -> bool:
        ...
        if len(arr) == 1 and root:
            return not root.left and not root.right and root.val == arr[0]
